...
import edu.nyu.tandon.query.PostHitsQueryEngine;
import edu.nyu.tandon.search.score.BM25Scorer;
 ...
 */
public class RawPostHits {

    private static final Logger LOGGER = LoggerFactory.getLogger(RawPostHits.class);
    ...
    private final PostHitsQueryEngine queryEngine;

    private long[] docHits;
    private static long queryCount = 0;

    public RawPostHits(final PostHitsQueryEngine queryEngine) {
    ...
    public int output( final ObjectArrayList<DocumentScoreInfo<Long>> results, final DocumentCollection documentCollection, final BigList<? extends CharSequence> titleList, final Marker marker ) throws IOException {
        int i;
        DocumentScoreInfo<Long> dsi;

        if (results == null) {
            output.printf("q:repeat\n");
            return 0;
        }

        if (displayMode == OutputType.HITS && results.size()>0) System.err.print("\r" + queryCount );

        for (int j=0; j<queryEngine.currTerms.size(); j++) {
            boolean found = false;
            Long pos = 1L << j;
            for (i = 0; i < results.size(); i++) {
                ...
                if ((dsi.info & pos) != 0) {
                    if (!found) output.printf("%d ", queryEngine.currTerms.get(j));
                    output.printf("%d ", document);
                    found = true;
                }
            }
            if (found) output.printf("\n");
        }
        return results.size();
    }
    ...
    public static void main( final String[] arg ) throws Exception {
        ...
        SimpleJSAP jsap = new SimpleJSAP( RawPostHits.class.getName(), "Loads indices relative to a collection, possibly loads the collection, and answers to queries.",
                new Parameter[] {
        ...
        RawPostHits.loadIndicesFromSpec(basenameWeight, loadSizes, documentCollection, indexMap, index2Weight);
		...
		*/
        ...
        final PostHitsQueryEngine queryEngine = new PostHitsQueryEngine( simpleParser, new DocumentIteratorBuilderVisitor( indexMap, index2Parser, indexMap.get( indexMap.firstKey() ), MAX_STEMMING ), indexMap );
        ...
        RawPostHits query = new RawPostHits( queryEngine );

        query.displayMode = OutputType.HITS;
        query.maxOutput = jsapResult.getInt("results", 10000);

        if (jsapResult.userSpecified("divert")) query.interpretCommand("$divert " + jsapResult.getObject("divert"));
        ...
        try {
            ...
            final ObjectArrayList<DocumentScoreInfo<Long>> results = new ObjectArrayList<DocumentScoreInfo<Long>>();
            ...
            for ( ;; ) {
                ...
                if (q.compareTo(lastQ) != 0) {
                    ...
                    query.output( null, documentCollection, titleList, TextMarker.TEXT_BOLDFACE );
                }
                lastQ = q;
            }
        }
