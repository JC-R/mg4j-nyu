...
class FeatureJoinTest extends FunSuite with BeforeAndAfterAll {
  ...
  test("join") {
    ...
    new Files {
      ...
      // then
      assertResult(1) {
        joined.filter(joined("id") === 0).count()
      }
      assertResult(2) {
        joined.filter(joined("id") === 1).count()
      }
      assertResult(Row.fromTuple((0, 0.1, 0.3))) {
        joined.filter(joined("id") === 0).first()
      }
      assertResult(Row.fromTuple((1, 0.2, 0.4))) {
        joined.filter(joined("id") === 1).first()
      }
      assertResult(Row.fromTuple((1, 0.2, 0.5))) {
        joined.filter(joined("id") === 1).take(2)(1)
      }
      assertResult(0) {
        joined.filter(!joined("id").isin(0, 1)).count()
      }
    }
  }
