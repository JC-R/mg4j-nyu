...
public class TrecQuery {
     ...
     */
    public Query.OutputType displayMode = Query.OutputType.SHORT;
    /**
     * The current output stream, changeable with <samp>$divert</samp>.
     */
    public PrintStream output = System.out;
    /**
     * Current topic number, for {@link Query.OutputType#TREC} only.
     */
    protected int trecTopicNumber;
    /**
     * Current run tag, for {@link Query.OutputType#TREC} only.
     */
    protected String trecRunTag;

    public TrecQuery(final QueryEngine queryEngine) {
    ...
    public static void main(final String[] arg) throws Exception {
		...
		*/
        ...
        // custom TREC mode
        query.displayMode = jsapResult.userSpecified("trec") ? Query.OutputType.TREC : Query.OutputType.TIME;
    ...
    public boolean interpretCommand(final String line) {
        ...
        try {
            command = Query.Command.valueOf(part[0].toUpperCase());
        ...
        switch (command) {
            case MODE:
                if (part.length >= 2) {
                    try {
                        final Query.OutputType tempMode = Query.OutputType.valueOf(part[1].toUpperCase());

                        if (tempMode != Query.OutputType.TREC && part.length > 2) System.err.println("Extra arguments.");
                        else if (tempMode == Query.OutputType.TREC && part.length != 4)
                            System.err.println("Missing or extra arguments.");
                        else {
                            displayMode = tempMode;
                            if (displayMode == Query.OutputType.TREC) {
...
//                output.printf( " [%.3f]", dsi.score );
                ...
                if ((displayMode == Query.OutputType.LONG || displayMode == Query.OutputType.SNIPPET) && dsi.info != null && queryEngine.intervalSelector != null) {
                    ...
                    for (Index index : sortedIndex)
                        if (index.hasPositions) {
                            ...
                            else if (displayMode == Query.OutputType.LONG || documentCollection == null)
        ...
        if (displayMode != Query.OutputType.TREC) output.println();
        return i;
    }
