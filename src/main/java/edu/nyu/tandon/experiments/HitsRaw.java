 ...
 */
public class HitsRaw {


	public final static int MAX_STEMMING = 2048;
	private static final Logger LOGGER = LoggerFactory.getLogger(HitsRaw.class);
	/**
	...
	/**
	 * Current topic number, for {@link BinnedRawHits.OutputType#TREC} only.
	 */
	private int trecTopicNumber;
	/**
	 * Current run tag, for {@link BinnedRawHits.OutputType#TREC} only.
	 */
	...
	// create and init binIndex lookup
	private static void buildBins(int maxResults) {

		// build static bin # lookup
		binIndex = new byte[maxResults];
		for (int i = 1; i <= maxResults; i++) binIndex[i - 1] = rank2bin(i);
	}
	...
	public static void main(final String[] arg) throws Exception {
		...
		SimpleJSAP jsap = new SimpleJSAP(HitsRaw.class.getName(), "Loads indices relative to a collection, possibly loads the collection, and answers to queries.",
				new Parameter[]{
		...
		HitsRaw.loadIndicesFromSpec(basenameWeight, loadSizes, documentCollection, indexMap, index2Weight);
		...
		*/
		...
		HitsRaw query = new HitsRaw(queryEngine);
