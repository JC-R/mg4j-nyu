 ...
 */
public class BinnedRawHits_grouped {


	static class Tuple implements Comparator<BinnedRawHits_grouped.Tuple> {
		public int term;
		public int doc;
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(BinnedRawHits_grouped.class);
	/**
	...
	public BinnedRawHits_grouped(final QueryEngine queryEngine) {
	...
	private static void dumpPosthits(BinnedRawHits_grouped query, boolean endRun) {
		...
		Tuple lastT = new Tuple(t0.term, t0.doc, t0.binIndex);
	...
	private static void dumpBatch(BinnedRawHits_grouped query, long numDocs, boolean endRun) {
	...
	public static void main(final String[] arg) throws Exception {
		...
		SimpleJSAP jsap = new SimpleJSAP(BinnedRawHits_grouped.class.getName(), "Loads indices relative to a collection, possibly loads the collection, and answers to queries.",
				new Parameter[]{
		...
		BinnedRawHits_grouped.loadIndicesFromSpec(basenameWeight, loadSizes, documentCollection, indexMap, index2Weight);
		...
		*/
		...
		BinnedRawHits_grouped query = new BinnedRawHits_grouped(queryEngine);
		...
		try {
			...
			for (; ; ) {
				...
				for (int i = 0; i < results.size(); i++) {
					...
					docHits[(doc * limits.length) + idx]++;     // docHits
					...
					for (int j = 0; j < dsi.info.size(); j++) {
						...
						postHits.add(new Tuple(term, doc, idx));
					}
				}
