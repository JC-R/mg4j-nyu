...
public class RandomAccess {
    ...
    public static void main( final String[] arg ) throws Exception {
        ...
        final long numberOPostings = jsapResult.getInt("random", (int)1000);
        ...
        for (int i=0; i<numberOPostings; i++) {
            ...
            System.err.print("\rAdding random postings: " + i);
        }
        Integer[] docs = set.toArray(new Integer[0]);

        // find 10 terms with at least t postings
        ...
        for (int k=0; k<numTerms; k++) {
            ...
            try {
                ...
                total /= 10;
                totSequential += total;
                System.out.println("Term " + k + ": " + list.frequency() + " postings");
                System.out.println("Sequential access:  " + total / 1000000. + " ms,  " + Util.format((list.frequency() * 1000000000.0) / total) + " postings/s,   " + Util.format( total / (double)list.frequency() ) + " ns/posting" );
                ...
                System.out.println("Random access:  " + docs.length + " lookups,  " + hits + " hits,  " + misses + " misses,  " + total / 1000000. + " ms,  " + Util.format((docs.length * 1000000000.0) / total) + " postings/s, " + Util.format( total / (double)docs.length ) + " ns/posting" );
                System.out.println();
            }
            catch( Exception e ) {
        ...
        System.out.println();
        System.out.println();
        System.out.println("Average: " + + totSeqPostings + " postings");
        System.out.println("Sequential access: "  + totSequential / 1000000. + " ms; " + Util.format((totSeqPostings * 1000000000.0) / totSequential) + " postings/s, " + Util.format( totSequential / (double)totSeqPostings ) + " ns/posting" );
        System.out.println("Random access: " + docs.length+ " lookups,  " + totHits + " hits,  " + totMisses + " misses,  " + totRandom / 1000000. + " ms,  " + Util.format((totRandPostings * 1000000000.0) / totRandom) + " postings/s, " + Util.format( totRandom / (double)docs.length ) + " ns/posting" );
