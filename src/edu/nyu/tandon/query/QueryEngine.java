...
public class QueryEngine<T> implements FlyweightPrototype<QueryEngine<?>> {
    ...
    /**
     * The current scorer, or <code>null</code> if no scorer is in use.
     */
    public Scorer scorer;
    /**
     * The builder visitor used to make queries into document iterators.
     */
    protected QueryBuilderVisitor<DocumentIterator> builderVisitor;
    /**
     * A transformer that will be applied to queries before resolving them, or <code>null</code>.
     */
    ...
    public synchronized void equalize(final int samples) {
        ...
        if (!(scorer instanceof AbstractAggregator))
     ...
     *
     * @param transformer a {@linkplain QueryTransformer query transformer}, or <code>null</code> to disable query transformation.
     */
    ...
    protected String multiplex(final String query) {
    ...
    public int process(final String queries, int offset, final int length, final ObjectArrayList<DocumentScoreInfo<T>> results) throws QueryParserException, QueryBuilderVisitorException, IOException {
