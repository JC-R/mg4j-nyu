 ...
 * 
 * <p>Unless a serialised {@link DocumentSequence} is specified using
 * the suitable option, an implicit {@link InputStreamDocumentSequence}
 ...
 * {@link DocumentFactory.FieldType#VIRTUAL} fields) indexing is performed on a virtual document
 ...
 * Usually, some edu.nyu.tandon.experiments and a look at the logs is all that suffices to find out good parameters
 ...
 * seen as a {@link DocumentalCluster}. Besides the {@linkplain Index.PropertyKeys standard keys},
 * the file contains {@link IndexCluster.PropertyKeys#LOCALINDEX} entries specifing the basename
 * of the batches, and a {@linkplain IndexCluster.PropertyKeys#STRATEGY strategy}. After creating manually suitable term maps for
...
public class Scan {
	...
	/** The extension of the UTF-8 file of titles. */
	public static final String TITLES_EXTENSION = ".titles";
	/**
	 ...
	 */
	public static final int BYTE_ARRAY_POSTING_LIST_INITIAL_SIZE = 8;
	;
	/**
	 * The frequency with which we report the current number of terms.
	 */
	public static final int TERM_REPORT_STEP = 1000000;
	/**
	...
	public final IOFactory ioFactory;
	...
	public Object2ReferenceOpenHashMap<MutableString, ByteArrayPostingList> termMap;
	/**
	 ...
	 * @see #run(String, DocumentSequence, Completeness, TermProcessor, DocumentCollectionBuilder, int, int, int, int[], VirtualDocumentResolver[], int[], String, long, String)
	 */
	@Deprecated
	...
	public static void run(final String basename, final DocumentSequence documentSequence, final Completeness completeness, final TermProcessor termProcessor, final DocumentCollectionBuilder builder, final int bufferSize,
		...
		run(IOFactory.FILESYSTEM_FACTORY, basename, documentSequence, completeness, termProcessor, builder, bufferSize,
			documentsPerBatch, maxTerms, indexedField, virtualDocumentResolver, virtualGap, mapFile, logInterval, tempDirName, null );
	}
	...
	public static void run( final IOFactory ioFactory, final String basename, final DocumentSequence documentSequence, final Completeness completeness, final TermProcessor termProcessor, final DocumentCollectionBuilder builder, final int bufferSize,
			final int documentsPerBatch, final int maxTerms, final int[] indexedField, final VirtualDocumentResolver[] virtualDocumentResolver, final int[] virtualGap, final String mapFile, final long logInterval,
			final String tempDirName, Set<String> cluster ) throws ConfigurationException, IOException {
		...
		while ( ( document = iterator.nextDocument() ) != null ) {
			...
			/*
			 * Skip documents that are not on the list.
			 */
			if (cluster != null && document.title() != null) {
				if (!cluster.contains(document.title())) {
					continue;
				} else {
					cluster.remove(document.title());
				}
			}
		...
		for (int i = 0; i < virtualDocumentGapSpec.length; i++) {
			...
			if (split >= 0) {
				...
				try {
					...
					if (virtualDocumentGap[indexedFields.indexOf(field)] < 0)
		...
		if (indexedFieldName.length == 0) {
			for (int i = 0; i < factory.numberOfFields(); i++) {
				...
				if (allSupported) indexedFields.add(i);
					...
					LOGGER.warn("Virtual field " + factory.fieldName(i) + " is not being indexed; use -a or explicitly add field among the indexed ones");
			}
		} else {
	...
	private static Set<String> parseCluster(String file) {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
			...
			LOGGER.error(String.format("Cluster file `%s` does not exist or could not be opened: continuing without cluster.", file));
			return null;
		}
	}
	...
	public static void main(final String[] arg) throws JSAPException, InvocationTargetException, NoSuchMethodException, ConfigurationException, ClassNotFoundException, IOException,
		...
		SimpleJSAP jsap = new SimpleJSAP(
				...
				new Parameter[]{
						...
						new FlaggedOption("cluster", JSAP.STRING_PARSER, JSAP.NO_DEFAULT, JSAP.NOT_REQUIRED, 'C', "cluster")
				});
		...
		final Set<String> cluster = parseCluster(jsapResult.getString("cluster"));
	 ...
	 */
	public long dumpBatch() throws IOException, ConfigurationException {
	...
	public void openSizeBitStream() throws IOException {
		if (!indexingIsVirtual)
	...
	protected static class PayloadAccumulator {
		 ...
		 */
		public void writeData() throws IOException, ConfigurationException {
