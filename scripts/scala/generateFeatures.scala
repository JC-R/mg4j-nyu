...
val toInt = udf((s: String) => s match {
	case null => 0
    case s => s.toInt
})
val toDouble = udf((s: String) => s match {
	case null => 0
    case s => s.toDouble
})
...
"p_b1","p_b2","p_b3","p_b4","p_b5","p_b6","p_b7","p_b8","p_b9","p_b10","p_b20","p_b40","p_b80","p_b160","p_b320","p_b640","p_b1280",
"p_top10","p_top20","p_top40","p_top80","p_top160","p_top1k").
withColumn("label_top1k",toInt($"label_top1k")).
withColumn("label_top10",toInt($"label_top10")).
...
val data = sqlContext.read.parquet(work_dir+model_name+".ph.train.parquet").na.fill(0.0)
	.map(row => { 
		val d = (for (i<-1 to 55) yield row.getDouble(i)).toArray
		LabeledPoint(row.getDouble(0), Vectors.dense(d).toSparse) 
	}).repartition(20).randomSplit(Array(0.7, 0.3), seed = 11L)
...
MLUtils.saveAsLibSVMFile(test,work_dir+model_name+".ph.test.libsvm")
