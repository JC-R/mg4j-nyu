...
#
# Requires: DocumentPruningStrategies must exist in serialized format
...
#
if [ "$#" -ne 4 ]; then
    echo "Illegal number of parameters: DocumentPrunedIndex.sh CORPUS STRATEGY_DIR BASELINE_INDEX MODEL"
    exit 1
fi
...
for n in 01 02 03 04 05 10; do
fname=$CORPUS-$MODEL-$n
$MY_PATH/SingletonDocumentPrunedIndex.sh $STRATEGY_DIR/$fname $FULL_INDEX &
done
wait
